...
apply gain to 89dB target (in steps of 1.5dB), save to output dir,
and compare results against mp3gain.exe reference values.
...
OUTPUT_DIR = Path("c:/tmp/mp3/89dbclaude")
TARGET_DB = 89.0
...
REFERENCE = {
    "1.mp3": dict(mp3gain_steps=-6, raw_db=-9.21, max_amp=33973.51, max_gain=210, min_gain=82),
    "2.mp3": dict(mp3gain_steps=-4, raw_db=-6.29, max_amp=32255.55, max_gain=210, min_gain=76),
    "3.mp3": dict(mp3gain_steps=-6, raw_db=-9.10, max_amp=35503.78, max_gain=210, min_gain=38),
    "4.mp3": dict(mp3gain_steps=-6, raw_db=-9.02, max_amp=34830.25, max_gain=210, min_gain=47),
    "5.mp3": dict(mp3gain_steps=-7, raw_db=-11.21, max_amp=34677.88, max_gain=188, min_gain=118),
    "6.mp3": dict(mp3gain_steps=-5, raw_db=-6.99, max_amp=38316.25, max_gain=210, min_gain=103),
    "7.mp3": dict(mp3gain_steps=-7, raw_db=-9.85, max_amp=35637.46, max_gain=187, min_gain=117),
}
...
def main() -> None:
    ...
    print(f"{'File':<12} {'Volume':>7} {'RawdB':>8} {'Steps':>6} {'AppliedDB':>10} "
          ...
          f"{'vs ref raw_db':>14} {'vs ref steps':>13}")
    print("-" * 110)
    ...
    for fname in sorted(REFERENCE.keys()):
        ...
        status = "OK" if (abs(delta_db) < 0.15 and steps_match) else "MISMATCH"
        if status != "OK":
            all_ok = False
        ...
        print(
            ...
            f"{delta_db:>+14.3f} {steps_match!s:>13}  {status}"
        )
