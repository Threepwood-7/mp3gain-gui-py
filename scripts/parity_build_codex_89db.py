...
def _build_one(src_path: Path, dst_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "min_gain": min_gain,
        "max_gain": max_gain,
        "track_peak": round(track_peak, 6),
    }


def _build_one_worker(src_text: str, dst_text: str) -> dict[str, object]:
    return _build_one(Path(src_text), Path(dst_text))


def _tool_signature_payload() -> dict[str, object]:
...
def _load_cache(expected_signature_hash: str) -> dict[str, dict[str, object]]:
    path = _cache_path()
    if not path.exists():
        return {}
    try:
...
def main() -> int:
    ...
    for src in source_files:
        ...
        if not args.force_rebuild and entry is not None and dst.exists():
            ...
            if cached_in == input_sha and cached_sig == signature_hash and cached_out:
                if sha256_file(dst) == cached_out:
    ...
    if pending:
        with ProcessPoolExecutor(max_workers=min(jobs_used, len(pending))) as executor:
            future_map = {
                executor.submit(_build_one_worker, str(src), str(dst)): src.name
    ...
    for src in source_files:
        result = results_by_name[src.name]
    ...
    report = {
        ...
        "rebuilt_files": sum(1 for item in results if not item["skipped"]),
        "skipped_files": sum(1 for item in results if item["skipped"]),
        "results": results,
    }
